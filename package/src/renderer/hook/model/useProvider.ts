...
export async function refreshProviders() {
  try {
    const list = await API.app.provider.list();
...
export async function createProvider(payload: ProviderInsert) {
  const record = await API.app.provider.create(payload);
  if (record) {
...
export async function updateProvider(
  ...
  const record = await API.app.provider.update(id, changes);
  if (record) {
    useProvidersStore.getState().upsertProvider(record);
  } else {
...
export async function deleteProvider(id: string) {
  await API.app.provider.delete(id);
  useProvidersStore.getState().removeProvider(id);
}

export async function fetchRemoteModels(providerId: string): Promise<RemoteModel[]> {
  return API.app.provider.fetchModels(providerId);
}

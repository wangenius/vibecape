...
export async function refreshModels() {
  try {
    const list = await window.api.app.model.list();
...
export async function createModel(payload: ModelInsert) {
  const record = await window.api.app.model.create(payload);
  if (record) {
...
export async function updateModel(id: string, changes: Partial<ModelInsert>) {
  const record = await window.api.app.model.update(id, changes);
  if (record) {
    useModelsStore.getState().upsertModel(record);
  } else {
    await refreshModels();
...
export async function deleteModel(id: string) {
  await window.api.app.model.delete(id);
...
export async function getDefaultModelId(
  ...
  try {
    return (
      useDefaultModelsStore.getState().defaultModels[category]?.model_id || null
    );
  } catch (error) {
...
export async function updateDefaultModel(
  ...
  const result = await window.api.app.settings.update(

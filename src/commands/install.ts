...
export const installCommand = new Command('install')
  ...
  .argument('[integration]', '要安装的集成 (auth, payments, i18n, database, email, storage, analytics)')
  ...
  .action(async (integration: string, options: any) => {
    try {
      ...
      // 获取提供商特定的配置选项
      const configOptions = await integrationManager.getConfigOptions(integration, selectedProvider);
      ...
      // 安装集成
      const spinner = ora(`正在安装 ${integration} (${selectedProvider})...`).start();
      ...
      const nextSteps = await integrationManager.getNextSteps(integration, selectedProvider);

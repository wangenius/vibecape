 ...
 * 工作区类型定义
 *
 * 用户目录: ~/vibecape/repositorys.json
 * 工作区目录: {docs_root}/{repository_id}/
 */
...
// ==================== 工作区索引 (~/vibecape/repositorys.json) ====================
...
/**
 * 工作区索引
 * 存储在 ~/vibecape/repositorys.json
 */
export type RepositorysIndex = {
  /** 文档根目录路径，所有工作区都存储在此目录下 */
  docs_root: string;
  /** 当前活动工作区 ID，null 表示无活动工作区 */
  current: string | null;
  /** 最近访问的工作区列表，按访问时间排序 */
  recent: RepositoryEntry[];
};
...
/** 默认工作区索引 */
export const DEFAULT_REPOSITORYS_INDEX: RepositorysIndex = {
